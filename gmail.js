const PORTFOLIO_CONTACT_EMAIL = "taonakha2018@gmail.com";

async function sendContactEmail(payload) {
    const { name, email, message } = payload;
    return fetch(`https://formsubmit.co/ajax/${PORTFOLIO_CONTACT_EMAIL}`, {
        method: "POST",
...
if (form) {
    form.addEventListener("submit", async e => {
        ...
        try {
            const emailRes = await sendContactEmail(payload);
            if (emailRes.ok) {
                recordSend();
                if (feedback) {
